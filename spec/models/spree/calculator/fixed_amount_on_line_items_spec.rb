...
RSpec.describe(Spree::Calculator::FixedAmountOnLineItems) do
  ...
  # Persistence is required: compute sorts by id so the last item can absorb
  # the rounding remainder, and FactoryBot line items need a variant.
  ...
  describe '#compute' do
    it('splits evenly across equal line items') do
      expect(shares(order_with_amounts(50, 50))).to eq([BigDecimal('5'), BigDecimal('5')])
    end

    it('gives a single line item the whole amount') do
      expect(shares(order_with_amounts(50))).to eq([BigDecimal('10')])
    end

    it('weights the split by line item value') do
      expect(shares(order_with_amounts(75, 25))).to eq([BigDecimal('7.5'), BigDecimal('2.5')])
    end

    it('gives the last item the rounding remainder') do
      expect(shares(order_with_amounts(33.33, 33.33, 33.34)))
        .to eq([BigDecimal('3.33'), BigDecimal('3.33'), BigDecimal('3.34')])
    end

    it('sums reconciled shares to the configured amount') do
      expect(shares(order_with_amounts(33.33, 33.33, 33.34)).sum).to eq(BigDecimal('10'))
    end

    it('returns amounts rounded to two decimal places') do
      expect(shares(order_with_amounts(10, 20, 70))).to all(satisfy { |share| share == share.round(2) })
    end
    ...
    context 'when preferred_amount has more than 2 decimal places' do
      ...
      it('produces shares rounded to at most 2 decimal places') do
        expect(shares(order_with_amounts(50, 50))).to all(satisfy { |share| share == share.round(2) })
      end
    end
    ...
    it('returns 0 when given nil') { expect(calculator.compute(nil)).to eq(0) }
    ...
    context 'when the currency does not match' do
      ...
      it('returns 0') { expect(shares(order_with_amounts(50, 50))).to eq([0, 0]) }
    end
    ...
    context 'when rounding the earlier shares would overshoot the budget' do
      ...
      it('never produces a negative share') do
        expect(shares(order_with_amounts(3.33, 3.33, 3.33, 3.33, 3.33, 3.33, 0.01))).to all(be >= 0)
      end
      ...
      it('accepts a small shortfall rather than a surcharge') do
        ...
        expect(shares(order_with_amounts(3.33, 3.33, 3.33, 3.33, 3.33, 3.33, 0.01)).sum)
          .to eq(BigDecimal('9.97'))
      end
    end
    ...
    context 'when the fixed amount exceeds the actionable total' do
      ...
      it('clamps the total discount to the line items total') do
        expect(shares(order_with_amounts(20, 10)).sum).to eq(BigDecimal('30'))
      end

      it('never discounts a line item by more than its own amount') do
        expect(shares(order_with_amounts(20, 10))).to eq([BigDecimal('20'), BigDecimal('10')])
      end
    end
  end

  describe '#compute when promotion rules exclude items' do
  ...
  describe '#compute when apply_only_on_full_priced_items is set' do
  ...
  describe '.description' do
    it('is translated') { expect(described_class.description).to eq('Fixed Amount (spread across line items)') }
  end
end
